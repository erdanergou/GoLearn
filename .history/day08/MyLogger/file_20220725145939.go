...
func (f *FileLogger) initFile() error {
	...
	// }
	go f.writeLogBackGround() // 开启一个goroutine执行后台写
	return nil
}
...
func (l *FileLogger) writeLogBackGround() {
	for {
		...
		case logTmp := <-l.logChan:
			// 把日志拼出来
			logInfo := fmt.Sprintf("[%s] [%s] [%s: %s: %d] %s\n", logTmp.timeStamp, getLogString(logTmp.Level), logTmp.funcName, logTmp.fileName, logTmp.line, fotmat)
