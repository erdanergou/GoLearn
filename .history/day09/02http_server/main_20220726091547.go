...
func f_Get(rw http.ResponseWriter, re *http.Request) {
	// 对于get请求，参数在url上
	queryParam := re.URL.Query()
	username := queryParam.Get("username")
	pwd := queryParam.Get("password")
	fmt.Println(username, pwd) //自动帮助识别url中的参数
	fmt.Println(re.URL)
...
func f_Post(rw http.ResponseWriter, re *http.Request) {
	// 对于post请求，参数在body上
