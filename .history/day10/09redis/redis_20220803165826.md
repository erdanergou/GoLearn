...
### Hash 适合存储对象:
	...
	hincrby key filed increment 为哈希表key中的域field的值加上增量1 -1
...
### Zset有序列表 
	是一个没有重复元素的字符串集合，每个成员都关联了一个score评分，用于排序。集合的成员是唯一的，但是评分score可以重复

	zadd key score1 value1 score2 value2 将一个或多个member元素及其score值加入到有序集key中

	zrange key start stop [withscores]返回有序集key中，下标在start stop之间的元素 带withscores可以让分数与值一起返回