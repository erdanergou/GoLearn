...
//类型别名只在代码编写过程中有效，编译完成后就不存在，内置的byte和rune都属于
...
func main() {
	...
	// 序列化
	// 1.结构体内部的字段首字母要大写，不大写别人访问不到
	// 2.反序列化时要传递指针
	p := point{
		X: 1,
		Y: 2,
	}
	pp, err1 := json.Marshal(p)
	if err1 != nil { //如果出错了（有错误err有值）
	...
	// 反序列化:由字符串 --> go中的结构体变量
	str1 := `{"first":12,"second":23}`
	var p3 point //构造一个变量准备接受反序列化
	err2 := json.Unmarshal([]byte(str1), &p3)
	if err2 != nil {
