...
func main() {
	...
	// db.Model(&mytable.Person{}).Preload("Job").Find(&ps)
	// fmt.Println(ps)
	// fmt.Println(u)
	...
	// join连接
	// db.Joins("inner join job on job.id = person.jobid").First(&u)
	// fmt.Println(u)

	//limit offset
	// db.Model(&mytable.Person{}).Limit(3).Order("age").Find(&ps)   // SELECT * FROM `person`   ORDER BY `age` LIMIT 3
	// db.Model(&mytable.Person{}).Limit(5).Offset(3).Find(&ps) //SELECT * FROM `person`   LIMIT 5 OFFSET 3  跳过前三个
	// fmt.Println(ps)
